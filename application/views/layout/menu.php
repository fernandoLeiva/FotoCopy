...
<aside class="main-sidebar">
    ...
    <section class="sidebar" data-widget="tree">
        ...
        <!-- Sidebar Menu -->
        <?php
        echo $this->multi_menu->inject_item('<li class="header">Panel de Navegación</li><li><a href="#" onClick="cargarView(\''.$paginaInicial.'\', \'index\', \'View\')" data-permission="View-"><i class="fa fa-dashboard"></i><span>Escritorio</span></a></li>', 'first')
                              ->render();
...
$(".sidebar .sidebar-menu a").click(function(event) {
    ...
    var permission  = "Add-Edit-Del-View"//$(this).data("permission");
...
function cargarView(controller, action, actions) {
  //  WaitingOpen();
    $('#content').empty();
    $("#content").load("<?php echo base_url(); ?>"+controller+"/"+action+"/"+actions);
    //WaitingClose();
}
</script>
